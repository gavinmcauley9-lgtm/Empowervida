...
// 1. Replace post 9 with this fully fleshed out content
const startPost9 = fileContent.indexOf("  {\n    id: 9,");
if (startPost9 === -1) throw new Error("Could not find post 9 block");

const endPost9 = fileContent.search(/  \},\n  \{\n    id: 10,/);
if (endPost9 === -1) throw new Error("Could not find end of post 9 block clearly");

fileContent = fileContent.substring(0, startPost9) + updatedPost + fileContent.substring(endPost9 + 4);

fs.writeFileSync('src/data/posts.js', fileContent);
console.log('Successfully updated Blog Post 9!');
