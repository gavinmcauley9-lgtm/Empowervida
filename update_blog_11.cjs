...
const fileContent = fs.readFileSync('src/data/posts.js', 'utf8');
...
const updatedPost = `  {
    ...
    content: \`
      ...
      <p style="font-size: 1.1rem; line-height: 1.8; margin-bottom: 1.5rem;">
        Protect your endothelium. Flatten the spike. By optimizing your post-meal responses with movement and strategic supplementation, you safeguard your long-term cardiovascular and cognitive wealth.
      </p>
...
// Parse out and replace just id 11
const startPostIdStr = '  {\n    id: 11,';

// Split the content carefully
const startIndex = fileContent.indexOf(startPostIdStr);
if (startIndex === -1) {
    throw new Error("Could not find post ID 11 block start.");
}

// Find the beginning of post 12 to safely cut
let endSearchStr = '\\n  },\\n  {\\n    id: 12,';
let endIndex = fileContent.search(/\n  \},\n  \{\n    id: 12,/);

if (endIndex === -1) {
    // maybe it ends before another specific post id...
    throw new Error("Could not find the end of post ID 11 block safely.");
}

const prefix = fileContent.substring(0, startIndex);
const suffix = fileContent.substring(endIndex + 4); // right at the `\n  {\n    id: 12,` start

const newFileContent = prefix + updatedPost + suffix;
fs.writeFileSync('src/data/posts.js', newFileContent);
console.log('Successfully updated Blog Post 11!');
